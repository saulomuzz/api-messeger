...
O diretório `scripts/` contém o utilitário `zabbix_whatsapp_sender.py`, preparado para uso em ações ou notificações do Zabbix.  
Ele envia uma mensagem de texto via endpoint `/send` e, opcionalmente, anexa um gráfico renderizado pelo próprio Zabbix utilizando `/send-media`.
...
```bash
python3 scripts/zabbix_whatsapp_sender.py   --api-base-url "https://whatsapp-api.exemplo.com"   --api-token "TOKEN_API"   --zabbix-url "https://zabbix.exemplo.com"   --zabbix-token "TOKEN_ZABBIX"   --graph-id 12345   --period 7200   "+5511998765432"   "Alerta: uso de CPU elevado"
```
...
O script retorna `0` em caso de sucesso, `2` quando o envio de texto falha e `3` caso o envio do gráfico não seja concluído.
